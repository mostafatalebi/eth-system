...
pub struct Executor<N: Network, P: Provider<N> + Clone> {
    ...
    done_tx_ch: tokio::sync::mpsc::Sender<TxExecModel>,
...
impl<N: Network, P: Provider<N> + Clone>  Executor<N, P> {
    pub async fn new(provider: P, cache_db: CacheDB<WrapDatabaseAsync<AlloyDB<N, P>>>,
                     incoming_ch: tokio::sync::mpsc::Receiver<Transaction>,
                     done_tx_ch: tokio::sync::mpsc::Sender<TxExecModel>) -> Result<Self, CoreError> {
    ...
    pub async fn initialize_chain_id(&mut self) -> Result<(), CoreError> {
        match self.provider.get_chain_id().await {
            ...
            Err(e) => {
                Err(CoreError::FailedToRetrieveChainId(e.to_string()))
            }
        }
    }
    ...
    pub async fn start_worker(&mut self) {
        ...
        while let Some(tx) = self.incoming_ch.recv().await {
            ...
            if revm_tx.is_err() {
                error!("{}", CoreError::TxTypeCastFailed(revm_tx.err().unwrap().to_string()));
                continue;
            }
            ...
            if self.check_result(&tx.inner.hash(), &tx_result) {
                ...
                let tx_model = TxExecModel::new(tx.clone(), tx_result.as_ref().unwrap().clone(), account.unwrap().unwrap());
    ...
    pub async fn validate(&self, tx: &Transaction) -> Result<(), CoreError> {
        if let Err(e) = validate_tx_signature(tx) {
            return Err(CoreError::ValidationFailed(e.to_string()));
        }

        // confirm chain_id
        let tx_chain_id = ConsensusTransaction::chain_id(tx).unwrap_or(0u64);
        if self.chain_id != tx_chain_id {
            return Err(CoreError::ValidationFailed(format!("chain_id mismatch {:?} is not {:?}", self.chain_id, tx_chain_id)));
        }

        // sender nonce
        let current_nonce = self.provider.get_transaction_count(tx.from()).await;
        if current_nonce.is_err() {
            return Err(CoreError::ValidationFailed(format!("failed to retrieve nonce info {:?}", current_nonce.err())));
        } else {
            if let Err(e) = validate_nonce(tx.nonce(), current_nonce.unwrap()) {
                return Err(CoreError::ValidationFailed(format!("failed to validate nonce {:?}", e)));
            }
        }
        ...
        if curr_balance.is_err() {
            return Err(CoreError::ValidationFailed(format!("failed to retrieve balance info {:?}", curr_balance.err())));
        }
        ...
        if let Err(e) = validate_balance(curr_balance.unwrap(), tx.value(), alloy::primitives::U256::from(tx.gas_limit()), max_fee_per_gas) {
            return Err(CoreError::ValidationFailed(format!("failed to validate balance {:?}", e)));
        }

        // basic naive gas limit check
        if tx.gas_limit() < MIN_GAS_LIMIT {
            return Err(CoreError::ValidationFailed("gas limit insufficient".to_string()));
        }
